...
class Ui_Form(object):
    def setupUi(self, Form):
        self.model = load_model('model/demo.h5')
        self.flag = False
        self.camera = None 
        ...
        if ret:
            ...
            for (x, y, w, h) in faces:
                ...
                if prediction.max() < 0.7:
                        ...
                        self.label = self.class_names[int(np.argmax(prediction))]
        
                kh = customerDAO()
                cus = kh.findByid(self.label)
                if cus is not None:
                        ten = cus.get_tenkh()
                else:
                        ten = 'Unknow'
                        
                cv2.putText(frame_rgb, ten, (x + w + 10, y + h), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2, cv2.LINE_AA)
